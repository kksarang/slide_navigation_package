...
class SideNavigationBar extends StatefulWidget {
  ...
  static _SideNavigationBarState of(final BuildContext context) {
    final _SideNavigationBarState? state = context.findAncestorStateOfType<_SideNavigationBarState>();
    if (state == null) {
...
class _SideNavigationBarState extends State<SideNavigationBar> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: AnimatedSize(
        ...
        child: SizedBox(
          width: MediaQuery.of(context).orientation == Orientation.portrait ? 50 : 200,
  ...
  List<Widget> _generateItems() {
    List<Widget> _items = widget.items
        .asMap()
        .entries
        .map<SideNavigationBarItemWidget>((MapEntry<int, SideNavigationBarItem> entry) => SideNavigationBarItemWidget(
    ...
    return Border(
      right: BorderSide(
        ...
        color: _theme.dividerTheme.mainDividerColor ??
            (MediaQuery.of(context).platformBrightness == Brightness.light ? Colors.grey[700]! : Colors.white),
      ),
    );
  }
